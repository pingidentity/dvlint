class DaVinciUtil {
  ...
  getFlowLinterOptions() {
    const flowLinterNodes = this.getNodesByName(/_dvlint_/i);
    ...
    if (flowLinterOptions.includeRules && flowLinterOptions.excludeRules) {
      throw new Error("include-rules and exclude-rules cannot both be defined");
    }
  ...
  getAllSubFlows(flows) {
    ...
    flows?.forEach((flow) => {
      ...
      flowNodes?.forEach((node) => {
        if (node.data.properties.subFlowId?.value?.value) {
